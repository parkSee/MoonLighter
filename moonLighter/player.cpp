...
HRESULT player::init(string _objName, tagFloat _pos)
{
	...
	EFFECTMANAGER->addEffect("보우이펙트", "will_bow_hit", 0.05f, 20);
		...
		for (int i = 0; i < 4; ++i)
		{
			for (int j = 0; j < _enemyObject[i].size(); ++j)
			{
				...
				if (IntersectRect(&rcTemp, &_rcSword, &rcEnemy) && _isRcSwordOn == true)
				{
					...
					EFFECTMANAGER->play("숏소드이펙트", _enemyObject[i][j]->pos.x, _enemyObject[i][j]->pos.y);
					_isEnemyHit = true;
				}
				for (int k = 0; k < ARROW_MAX; ++k)
				{
					...
					if (IntersectRect(&rcTemp, &_arrow[k].rc, &rcEnemy))
					{
						...
						EFFECTMANAGER->play("보우이펙트", _enemyObject[i][j]->pos.x, _enemyObject[i][j]->pos.y);
						_isEnemyHit = true;
